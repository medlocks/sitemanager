...
import { View, Text, TouchableOpacity, Modal, ScrollView, SafeAreaView, Image, ActivityIndicator, Linking, StyleSheet } from 'react-native';
...
export const AuditDetailModal = ({ visible, item, onClose, viewMode }: Props) => {
  ...
  const fetchAssetHistory = async (assetId: string) => {
    try {
      ...
      const { data } = await supabase.from('maintenance_logs').select('*').eq('asset_id', assetId).order('service_date', { ascending: false });
      setHistory(data || []);
  ...
  const resolveEvidence = (item: any) => {
    ...
    if (rawPath && !rawPath.startsWith('file://')) {
      if (rawPath.startsWith('http')) {
        ...
        const { data } = supabase.storage.from('incident-evidence').getPublicUrl(rawPath);
        if (data?.publicUrl) setEvidenceUri(`${data.publicUrl}?t=${new Date().getTime()}`);
        setLoadingImg(false);
      }
    } else {
      setEvidenceUri(null);
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" accessibilityViewIsModal={true}>
      <SafeAreaView style={styles.safeArea} testID="audit-modal-detail">
        <View style={styles.modalHeader}>
          <Text style={styles.modalTitle} accessibilityRole="header">Statutory Record Detail</Text>
          <TouchableOpacity 
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          <View style={styles.detailSection}>
            ...
            <View style={styles.timelineItem}>
              ...
              <View style={styles.timeInfo}>
                ...
                <Text style={styles.timeValue}>{formatDate(item?.created_at || item?.date_time)}</Text>
              </View>
            </View>
            ...
            {(item?.status === 'Resolved' || item?.status === 'Compliant') && (
              <View style={[styles.timelineItem, { marginTop: SPACING.m }]}>
                ...
                <View style={styles.timeInfo}>
                  ...
                  <Text style={styles.timeValue}>{formatDate(item?.resolved_at || item?.last_service)}</Text>
                </View>
              </View>
            )}
          </View>
          ...
          <View style={styles.detailSection}>
            <Text style={styles.label} accessibilityRole="header">CORE DETAILS</Text>
            <Text style={styles.value} testID="audit-detail-title">{item?.asset_name || item?.description || item?.injured_person_name}</Text>
            <View style={styles.locationRow}>
              <Ionicons name="location" size={18} color={COLORS.textLight} />
              <Text style={styles.locationText}>{item?.location || 'Not Specified'}</Text>
            </View>
          ...
          <View style={styles.detailSection}>
            <Text style={styles.label} accessibilityRole="header">LATEST DIGITAL EVIDENCE</Text>
            {loadingImg ? (
              ...
              <View style={styles.noEvidence} accessibilityLabel="No valid cloud evidence attached">
          ...
          {viewMode === 'Assets' && (
            <View style={styles.detailSection}>
              <Text style={styles.label} accessibilityRole="header">MAINTENANCE HISTORY (TRAIL)</Text>
              {loadingHistory ? (
          ...
          {viewMode !== 'Assets' && (item?.status === 'Resolved' || item?.injury_description) && (
            <View style={styles.detailSection}>
              <Text style={styles.label} accessibilityRole="header">COMPLETION DATA</Text>
