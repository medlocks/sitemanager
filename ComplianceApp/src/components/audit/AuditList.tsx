...
export const AuditList = ({ data, viewMode, searchQuery, onSearchChange, onSelectItem }: Props) => {
  ...
  const renderItem = ({ item, index }: { item: any, index: number }) => {
    ...
    const statusLabel = item.status?.toUpperCase() || (isAccident ? 'LOGGED' : 'PENDING');
    const isCompliant = item.status === 'Resolved' || item.status === 'Compliant';
    ...
    return (
      ...
      >
        ...
        <View style={styles.locationContainer}>
          <Text style={styles.logLoc} accessibilityLabel={`Location: ${item.location}`}>
             {item.location}
          </Text>
        </View>
      </TouchableOpacity>
