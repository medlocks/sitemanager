...
describe("FileService", () => {
  ...
  it("should upload incident evidence online and return filename", async () => {
    ...
    mockUpload.mockResolvedValue({
      data: { path: "fault_123.jpg" },
      error: null,
    });

    const result = await fileService.uploadIncidentEvidence(
      "user123",
      "file://test-image.jpg",
    );
  ...
  it("should enqueue to incident-evidence bucket when offline", async () => {
    ...
    expect(result).toHaveProperty(
      "path",
      expect.stringMatching(/^fault_\d+\.jpg$/),
    );
  ...
  it("should throw error for invalid file types in incident evidence", async () => {
    await expect(
      fileService.uploadIncidentEvidence("user123", "file://test.pdf"),
    ).rejects.toThrow("Invalid file type. Only images (JPG, PNG) are allowed.");
  });
