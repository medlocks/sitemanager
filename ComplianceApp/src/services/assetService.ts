...
export const assetService = {
  ...
  async updateAsset(id: string, asset: Partial<Asset>) {
    ...
    if (asset.asset_name)
      cleanAsset.asset_name = InputValidator.sanitize(asset.asset_name);
    if (asset.location)
      cleanAsset.location = InputValidator.sanitize(asset.location);

    if (asset.next_service_due) {
      const nameToCheck = cleanAsset.asset_name || "Placeholder";
      const validation = InputValidator.validateAsset(
        nameToCheck,
  ...
  async duplicateAsset(item: any) {
    ...
    const { error } = await supabase.from("assets").insert([dataToInsert]);
