...
describe('ContractorService', () => {
  ...
  it('should fetch a single contractor profile', async () => {
    (supabase.from('') as any).single.mockResolvedValue({ data: { id: 'u1', name: 'John' }, error: null });
  ...
  it('should fetch all users with Contractor role', async () => {
    ...
    expect((supabase.from('') as any).eq).toHaveBeenCalledWith('role', 'Contractor');
  });

  it('should fetch only approved contractors', async () => {
    await contractorService.getApprovedContractors();
    expect((supabase.from('') as any).eq).toHaveBeenCalledWith('role', 'Contractor');
    expect((supabase.from('') as any).eq).toHaveBeenCalledWith('competence_status', 'Approved');
  });
  ...
  it('should update specialism online', async () => {
    ...
    expect(supabase.from).toHaveBeenCalledWith('profiles');
    expect((supabase.from('') as any).update).toHaveBeenCalledWith({ specialism: 'Electrical' });
  });
  ...
  it('should assign contractor to an asset when online', async () => {
    ...
    expect((supabase.from('') as any).update).toHaveBeenCalledWith(expect.objectContaining({
      assigned_to: 'cont_456'
    }));
  });
});