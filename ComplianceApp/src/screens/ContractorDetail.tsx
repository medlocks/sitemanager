    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll}>
        <View style={styles.profileHeader}>
          ...
          <Text style={styles.name} accessibilityRole="header">{contractor.name}</Text>
          <View style={styles.badge}>
            <Text style={styles.badgeTxt}>{contractor.specialism?.toUpperCase() || 'GENERAL'}</Text>
          </View>
          <Text style={styles.email} accessibilityLabel={`Email address: ${contractor.email}`}>
            {contractor.email}
          </Text>
          <Text 
        ...
        <View style={styles.actionSection}>
          {loading ? (
            <View style={styles.loaderContainer}>
              <ActivityIndicator testID="status-loading-indicator" color={COLORS.primary} size="large" />
            </View>
          ) : (
            <>
              <TouchableOpacity 
...
const styles = StyleSheet.create({
  ...
  badgeTxt: { color: COLORS.primary, ...TYPOGRAPHY.caption, fontWeight: '800', letterSpacing: 1 },
  email: { ...TYPOGRAPHY.body, color: COLORS.textLight },
  statusInfo: { ...TYPOGRAPHY.body, fontWeight: '800', marginTop: SPACING.xs },
  section: { marginBottom: SPACING.xl },
  label: { ...TYPOGRAPHY.caption, fontWeight: '800', color: COLORS.text, marginBottom: SPACING.s },
  fileBtn: { 
  ...
  actionSection: { borderTopWidth: 2, borderTopColor: COLORS.lightGray, paddingTop: SPACING.l },
  approveBtn: { 
  ...
  btnTxt: { color: COLORS.white, fontSize: 16, fontWeight: '900', letterSpacing: 1 },
